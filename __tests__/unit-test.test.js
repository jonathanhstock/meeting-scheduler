...
const WeeklySchedule = require("../server/WeeklySchedule.js");
const calculateSchedule = require("../server/CalculateSchedule.js");
const {
...
describe("WeeklySchedule", () => {
  ...
  test("addTimeSlot should add a valid time slot to the schedule", () => {
    ...
    expect(schedule.getTimeSlots("Mon")).toEqual([
...
describe("calculateSchedule", () => {
  ...
  // Check start time
  expect(
    mutualSchedule.getTimeSlots("Mon").map((slot) => slot.startTime)
  ).toEqual(expectedMon.map((slot) => slot.startTime));
  expect(
    mutualSchedule.getTimeSlots("Tue").map((slot) => slot.startTime)
  ).toEqual(expectedTue.map((slot) => slot.startTime));

  // Check end time
  expect(
    mutualSchedule.getTimeSlots("Mon").map((slot) => slot.endTime)
  ).toEqual(expectedMon.map((slot) => slot.endTime));
  expect(
    mutualSchedule.getTimeSlots("Tue").map((slot) => slot.endTime)
  ).toEqual(expectedTue.map((slot) => slot.endTime));
...
describe("API Tests", () => {
  ...
  describe("GET /schedules/:id", () => {
    it("should retrieve schedules for a valid user", async () => {
      const response = await request(app).get("/schedules/user1");
    ...
    it("should return error for an invalid user", async () => {
      const response = await request(app).get("/schedules/nonexistentuser");
